...
/**
 * Valida ORDER_COMPLETE_API_KEY (solo cabeceras: Bearer o X-Api-Key).
 */
function validateOrderCompleteApiKey(req) {
    const expected = safeText(process.env.ORDER_COMPLETE_API_KEY);
    if (!expected) {
        return { ok: false, status: 503, message: 'API no disponible: ORDER_COMPLETE_API_KEY no configurada' };
    }
    ...
    if (!token || !timingSafeEqual(token, expected)) {
        return { ok: false, status: 401, message: 'No autorizado: envie Authorization Bearer o X-Api-Key' };
    }
    return { ok: true };
}
