...
async function syncAuthUserCredentials(supabase, options) {
    ...
    if (anonKey) {
        try {
            ...
            const uid = signInData && signInData.user && signInData.user.id;
            if (!signInErr && uid) {
                const { error: updateExistingError } = await supabase.auth.admin.updateUserById(uid, {
                    password: authPassword,
                    app_metadata: appMetadata
                });
                if (!updateExistingError) {
                    await linkUserId(uid);
                    return { ok: true, authUserId: uid };
                }
                logError(label + ' updateUserById tras signIn', updateExistingError);
            }
        } catch (signInLookupErr) {
            logError(label + ' signInWithPassword lookup', signInLookupErr);
        }
    }
