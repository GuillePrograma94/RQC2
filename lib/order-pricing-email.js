...
function computeLinePricingSinIva(linea, catalogProduct, usuario, clavesMap, pactosMap, ofertasByCodigo, allLineas, intervalosCache, loteCache) {
    ...
    let precioNeto = pvpBase;
    if (usarOferta) precioNeto = precioConDescuento;
    else if (usarTarifa) precioNeto = priceDtoTarifa;

    return {
        pvp_unitario: roundMoney(pvpBase),
        precio_neto_unitario: roundMoney(precioNeto)
    };
}
...
async function resolveOrderEmailPricing(supabase, usuario, productos) {
    ...
    const productosAjustados = lineas.map(function (linea) {
        ...
        const cantidad = toNumber(linea.cantidad, 0);
        const pricing = computeLinePricingSinIva(
            linea,
            catalogProduct,
            usuario,
            clavesMap,
        ...
        const subtotal = roundMoney(pricing.precio_neto_unitario * cantidad);
        totalImporte += subtotal;
        return Object.assign({}, linea, {
            pvp_unitario: pricing.pvp_unitario,
            precio_neto_unitario: pricing.precio_neto_unitario,
            precio_unitario: pricing.precio_neto_unitario,
            subtotal: subtotal
        });
    });
