 ...
 * Prueba el endpoint de PVP usando GET /api/tienda/v1/articulos/pvp?codigo=XXX
...
module.exports = async (req, res) => {
    ...
    const codigo = req.query.codigo || '0004223500340';
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(`ERP PVP error ${response.status}: ${statusMessage}`);
        }

        res.status(200).json({
            success: true,
            message: 'Endpoint de PVP funcionando',
            codigo: codigo,
            data: data,
            url: url,
        ...
        res.status(502).json({ 
            success: false,
            message: error.message || 'Error al probar endpoint de PVP',
